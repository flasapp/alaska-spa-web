	...
	async function signup() {
		...
		if (resp.code) {
			...
			if (resp.code == "USER_EXISTS")
				return addToast({
					text: "Ha ocurrido un error con sus datos, intentelo nuevamente mas tarde",
					type: "Error",
				});
		}
		if (resp.success) {
	...
	<div class="flex-grow place-items-center">
		...
		<form action="" class="w-full">
			<div class="form-control">
				...
				<div class="w-full text-left mb-4">
					<span class="text-sm"
						>* Todos los campos son obligatorios</span
					>
				</div>
				<button
