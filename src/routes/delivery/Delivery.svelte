...
<InfoSection {title} {breadcrumbs}>
	...
	<ul class="ml-4 schedules">
		<li>
		11 a 14 hrs 
		<div class="tooltip tooltip-right" data-tip="Para este horario el pedido debe realizarse antes de las 10hrs.">
			<InfoIcon />
		</div>
		...
		<li>
		14 a 17 hrs 
		<div class="tooltip tooltip-right" data-tip="Para este horario el pedido debe realizarse antes de las 13hrs.">
			<InfoIcon />
		</div>
		</li>
		<li>
	...
	{:then}
		...
		{#each neighbourhoods as neighbourhood}
			...
			<div class="neigh-container p-4 bg-opacity-60 rounded-xl hover:bg-opacity-80 hover:shadow-md transition-all duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-102">
				<h3 class="text-md font-regular w-100 text-left product-title">{neighbourhood.title}</h3>
				<div class="w-full font-regular text-sm" style="letter-spacing: .5px;">$ {neighbourhood.amount}</div>
			</div>
		{/each}
		</div>
