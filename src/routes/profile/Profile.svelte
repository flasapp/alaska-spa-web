<script>
	...
	const save = async () => {
		...
		if (!$GeneralSettings.userLogged.lastName)
			return (invalidForm.lastName = true);
		if (!$GeneralSettings.userLogged.email)
			return (invalidForm.email = true);
		if (!$GeneralSettings.userLogged.phone)
			return (invalidForm.phone = true);
		if (!$GeneralSettings.userLogged.address.street)
			return (invalidForm.street = true);
		if (!$GeneralSettings.userLogged.address.number)
			return (invalidForm.number = true);
		if (!$GeneralSettings.userLogged.address.neighbourhood)
			return (invalidForm.neighborhood = true);
		...
		addToast({
			text: "Ha ocurrido un error, intentelo nuevamente mas tarde",
			type: "Error",
		});
	};
	...
	const getMyOrders = async () => {
		setTimeout(async () => {
			// --> This is a workaround if the user reload the page. The userLogged is not available yet
			if (!$GeneralSettings.userLogged.id) return;
			let resp = await get(
				`orders-user/${$GeneralSettings.userLogged.id}`,
			);
...
<div
	class="mx-auto mt-4 py-8 max-w-2xl px-4 sm:px-6 lg:max-w-7xl lg:px-8 relative text-center"
>
	...
	<div class="text-left">
		...
		<div>
			<div class="w-full lg:w-1/2 m-auto text-left mt-4">
				<div
					class="bg-gradient-to-r from-violet-400 to-blue-400 p-1 rounded"
				>
					<div class="bg-base-100 p-4">
						<div role="tablist" class="tabs tabs-boxed bg-base-100">
							<a
								role="tab"
								class="tab {active_tab == 'INFO'
									? 'tab-active'
									: ''}"
								on:click={() => {
									active_tab = "INFO";
								...
								{#key orders}
									{#if orders.length == 0}
										...
										>
											<InfoIcon />
											<span
												>No se encontraron pedidos.</span
											>
										</div>
												...
												<tbody>
													{#each orders as order}
														...
														>
															...
															<td class=""
																>{order.deliveryDateToDisplay}</td
															>
															<td class=""
																>{order.deliverySchedule}</td
															>
															<td class=""
																>$ {order.total}</td
															>
